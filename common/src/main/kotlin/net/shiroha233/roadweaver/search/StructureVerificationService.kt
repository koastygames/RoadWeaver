    ...
    fun verifyPredictedStructures(level: ServerLevel, predicted: List<Records.StructureInfo>?): List<Records.StructureInfo> {
        ...
        for (info in predicted) {
            ...
            for (placement in placements) {
                ...
                if (r.equals(StructureCheckResult.START_PRESENT)) {
                    best = r
                    break
                }
                if (r.equals(StructureCheckResult.CHUNK_LOAD_NEEDED)) {
                    best = r
                }
            }

            if (best.equals(StructureCheckResult.START_PRESENT) || best.equals(StructureCheckResult.CHUNK_LOAD_NEEDED)) {
                result.add(info)
            }
        }
