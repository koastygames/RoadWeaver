...
class StructureListWidget(
    ...
    height: Int,
    top: Int
) : ContainerObjectSelectionList<StructureListWidget.Entry>(minecraft, width, height, top, 24) {
                ...
                if (optMod.isPresent) {
                    ...
                    if (logoOpt.isPresent) {
                        ...
                        if (logoPath.startsWith("assets/")) {
                            ...
                            ResourceLocation.tryParse(logoPath)?.let { candidates.add(it) }
                        } else {
    ...
    override fun getScrollbarPosition(): Int = width - 10
    ...
    class ModHeaderEntry(
        ...
        override fun render(
            ...
            if (icon != null) {
                ...
                graphics.blit(icon, left + 14, top + 4, 0f, 0f, 16, 16, 16, 16)
        ...
        override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
            if (button == 0) {
                val left = list.rowLeft
                // 点击整个条目都可以切换折叠，体验更好
                if (mouseX >= left && mouseX <= left + list.rowWidth) {
    ...
    class TagEntry(
        ...
        override fun render(
            ...
            val boxColor = if (enabled) 0xFF00FF00.toInt() else 0xFFFF0000.toInt()
        ...
        override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
            if (button == 0) {
                ...
                // 箭头区域
                if (mouseX >= left + indent && mouseX <= left + indent + 10) {
                    onExpandToggle.accept(tag)
                    return true
                }
                
                // 剩余区域 toggle checkbox
                if (mouseX > left + indent + 10 && mouseX <= left + list.rowWidth) {
                    onToggle.accept(tag)
                    return true
                }
            }
