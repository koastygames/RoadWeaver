...
data class LootConfig(
    ...
    fun apply(level: ServerLevelAccessor, anchorPos: BlockPos, random: RandomSource): Boolean {
        // 概率检查
        if (chance.toDouble() < 1.0 && random.nextFloat().toDouble() > chance.toDouble()) {
            return false
        }
    ...
    companion object {
        ...
        val ROADSIDE_SUPPLIES = LootConfig(
            ResourceLocation("roadweaver", "chests/roadside_supplies"),
            Vec3i(0, 1, 0),
            1.0f
        )
        ...
        val CABIN_CHEST = LootConfig(
            ResourceLocation("roadweaver", "chests/cabin_chest"),
            Vec3i(0, 1, 0),
            1.0f
        )
        ...
        val RARE_LOOT = LootConfig(
            ResourceLocation("roadweaver", "chests/rare_loot"),
            Vec3i(0, 1, 0),
            0.2f
        )
    }
