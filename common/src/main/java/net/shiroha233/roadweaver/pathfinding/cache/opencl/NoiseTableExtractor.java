...
public final class NoiseTableExtractor {
    ...
    private int extractNormalNoise(NormalNoise noise) {
        PerlinNoise first = (PerlinNoise) DensityGraphReflection.read(noise, "first");
        PerlinNoise second = (PerlinNoise) DensityGraphReflection.read(noise, "second");
        ...
        double valueFactor = DensityGraphReflection.readDouble(noise, "valueFactor", Double.NaN);
    ...
    private int extractPerlinNoise(PerlinNoise noise) {
        Object levelsObject = DensityGraphReflection.read(noise, "noiseLevels");
        if (!(levelsObject instanceof ImprovedNoise[] levels)) {
            throw new UnsupportedOperationException("PerlinNoise missing noiseLevels");
        }
        Object amplitudesObject = DensityGraphReflection.read(noise, "amplitudes");
        ...
        int firstOctave = DensityGraphReflection.readInt(noise, "firstOctave", 0);
        double lowestFreqInputFactor = DensityGraphReflection.readDouble(noise, "lowestFreqInputFactor", Double.NaN);
        double lowestFreqValueFactor = DensityGraphReflection.readDouble(noise, "lowestFreqValueFactor", Double.NaN);
        double maxValue = DensityGraphReflection.readDouble(noise, "maxValue", Double.NaN);
    ...
    private int extractImprovedNoise(ImprovedNoise noise) {
        Object permutationObject = DensityGraphReflection.read(noise, "p");
