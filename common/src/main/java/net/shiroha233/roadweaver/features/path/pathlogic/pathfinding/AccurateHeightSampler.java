...
/**
 * 精确高度采样器：使用 ChunkGenerator#getBaseHeight 对指定 (x,z) 进行二次采样。
 *
 * 设计目标：
 * - 寻路阶段仍使用 FastHeightSampler（高性能）
 * - 路径确定后仅对“最终路径节点”精采样（可接受的开销），确保道路铺设高度准确
 */
public final class AccurateHeightSampler {
    ...
    /**
     * 道路“地表高度”语义：
     * - 陆地：取 MOTION_BLOCKING_NO_LEAVES（更贴近可行走地面）
     * - 水域：取 WORLD_SURFACE_WG（接近水面），供桥梁/浮标/跳过逻辑使用
     */
