...
import net.shiroha233.roadweaver.client.config.StructurePredictionDimensionWhitelistScreen;
...
public class OpenStructurePredictionDimensionWhitelistEntry extends AbstractConfigListEntry<Void> {
    ...
    public OpenStructurePredictionDimensionWhitelistEntry() {
        super(Component.translatable("config.roadweaver.open_structure_prediction_dimension_whitelist"), false);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0 && lastArea != null && lastArea.contains(mouseX, mouseY)) {
            ...
            if (mc != null) {
                mc.setScreen(new StructurePredictionDimensionWhitelistScreen(mc.screen));
            }
            return true;
        }
        return false;
